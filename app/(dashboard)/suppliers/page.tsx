...
const IMPORT_CONFIG: Record<string, { endpoint: string; needsFtp?: boolean; needsFile?: boolean }> = {
  ...
  'Kap-Horn':           { endpoint: '/api/import/kaphorn' },
}
...
function getLastSummary(s: Supplier): ImportSummary | null {
  return (s.sync_state?.last_import_summary as ImportSummary) ?? null
}

function saveImportSummary(supplierId: string, data: ImportProgress) {
  const summary: ImportSummary = {
    stage:     data.stage as 'done' | 'error',
  ...
  fetch(`/api/suppliers/${supplierId}`, {
    ...
    body: JSON.stringify({ sync_state: { last_import_summary: summary } }),
  })
}
  ...
  function startImport(supplier: Supplier, mode?: string) {
    ...
    es.onmessage = (e) => {
      ...
      if (data.stage === 'done' || data.stage === 'error') {
        ...
        setImporting(null)
        saveImportSummary(supplier.id, data)
        reloadSuppliers()
      }
    }
          ...
          {suppliers.map(s => {
            ...
            return (
              <div key={s.id} className="bg-white rounded-lg border border-gray-200 p-6">
                          ...
                          <button
                            ...
                            title={
                              s.name === 'Palby'
                                ...
                                : s.name === 'Kap-Horn'
                                ? 'Henter productfeed.xml fra Kap-Horn FTP, matcher via EAN og opdaterer data inkl. spec-PDFs. Ukendte produkter sendes til gennemgang.'
                ...
                {/* Sidst kendte resultat — fra DB (vises når ingen aktiv import i denne session) */}
                {lastImported !== s.id && importing !== s.id && lastSummary && (
                  <div className={`mt-4 border-t border-gray-100 pt-3 text-xs rounded px-3 py-2 ${
                    lastSummary.stage === 'error'
                    ...
                    <div className="flex gap-4 flex-wrap">
                      {lastSummary.stage !== 'error' && <>
                        <span>Matchet: <strong>{lastSummary.matched}</strong></span>

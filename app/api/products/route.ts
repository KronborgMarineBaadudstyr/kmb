...
// GET /api/products?search=&status=&category=&page=1&per_page=50&sort=name&order=asc
export async function GET(request: Request) {
  ...
  const order     = url.searchParams.get('order') === 'desc' ? false : true  // ascending
  ...
  // Tilføj primærbillede URL direkte på produktet
  const products = (data ?? []).map(p => {
    const imgs = (p.product_images as { url: string; alt_text: string | null; is_primary: boolean; position: number }[]) ?? []
    const primary = imgs.find(i => i.is_primary) ?? imgs.sort((a, b) => a.position - b.position)[0]
    return {
      ...p,
      primary_image_url: primary?.url ?? null,
      image_count:       imgs.length,
      product_images:    undefined,  // fjern nested array fra response
    }
  })
